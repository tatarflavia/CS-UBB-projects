...
//std::ifstream & operator>>(std::ostream & fin, WeddingDress & dress)
//{
//	/*int size = 0;
//	std::string colour;
//	float price = 0;
...
//	fin >> dress.size >> dress.colour >> dress.price >> dress.quantity >> dress.photograph;
...
std::ofstream & operator<<(std::ofstream & fout, WeddingDress & dress)
{
	fout << dress.get_size() << " " << dress.get_colour() << " " << dress.get_price() << " " << dress.get_quantity() << " " << dress.get_photograph();
	return fout;
}

std::ifstream & operator>>(std::ifstream & fin, WeddingDress & dress)
{
	fin >> dress.size >> dress.colour >> dress.price >> dress.quantity >> dress.photograph;
	return fin;
}
