...
void UI::administrator_run()
{
	...
	while (1)
	{
		...
		else
		{
			...
			try
			{
				if (input == 1)
				{
					...
					if (std::cin.fail())
					{
						cin.clear();
						cin.ignore(123, '\n');
						throw InputDataError("Please give an integer for the size!\n");
					}
					...
					if (std::cin.fail())
					{
						cin.clear();
						cin.ignore(123, '\n');
						throw InputDataError("Please give a floating point number for the price!\n");
					}
					...
					if (std::cin.fail())
					{
						cin.clear();
						cin.ignore(123, '\n');
						throw InputDataError("Please give an integer for the quantity!\n");
					}
			...
			{
				std::cout << e.what();
				std::cout << "Please try again entering data!\n";
			}
			catch (ValidatorError e)
			{
				std::cout << e.what();
				std::cout << "The info for the dress is invalid! Please try again!\n";
			}
			catch (RepoError e)
			{
				std::cout << e.what();
...
void UI::user_run()
{
	...
	while (1)
	{
		...
		else if (in == 2)
		{
			...
			std::cout << "The application will be opened shortly!\n";
			this->ctlr.get_basket().execute_application();
			}
...
int UI::read_an_int()
{
	...
	while (!(std::cin >> n))
	{
		...
		std::cin.clear();
		std::cin.ignore(123, '\n');
		std::cout << "Please give again your answer:";
	}
	return n;
}
